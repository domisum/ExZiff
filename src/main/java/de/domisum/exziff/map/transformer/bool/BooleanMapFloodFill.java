...
public class BooleanMapFloodFill implements Transformer<BooleanMap>
{
	...
	public BooleanMap transform(BooleanMap input)
	{
		...
		for(int y = 0; y < input.getHeight(); y++)
			for(int x = 0; x < input.getWidth(); x++)
				if((x == 0) || (y == 0) || (x == (input.getWidth()-1)) || (y == (input.getHeight()
						-1))) // only the pixels on the outer edge
	...
	private void tryAddNeighborPixel(
			BooleanMap input,
			Coordinate2DInt base,
			Collection<Coordinate2DInt> unvisitedPixels,
			boolean[][] processedPixels,
			int dX,
			int dY)
	{
		int nX = base.x+dX;
