...
public class ContinentsPolygonDeformer implements RandomizedGenerator<List<Polygon2D>, List<Polygon2D>>
{
	...
	private class GenerateMethodObject
	{
		...
		private boolean tryDeformPolygonSide(PolygonSide polygonSide, Vector2D orthogonalOutward)
		{
			...
			double offsetPointPosAlongSide = RandomUtil.distribute(0.5, maxDivisionBasePointCenterOffset, this.random);
			...
			boolean valid = polygonValidator.isPolygonValid(newPolygon, polygonSide.polygon, this.polygons);
			if(valid)
			{
